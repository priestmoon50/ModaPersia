...
const Register = () => {
  ...
  // Debounced function for checking field availability
  useEffect(() => {
    const debouncedCheck = _.debounce(async (field, value) => {
      ...
      try {
        ...
        field === "name" ? setNameError(exists ? "Name is already taken" : null) : setEmailError(exists ? "Email is already in use" : null);
      } catch {
        field === "name" ? setNameError("Error checking name availability") : setEmailError("Error checking email availability");
      } finally {
        setIsCheckingAvailability(false);
      }
    }, 500);

    if (name) debouncedCheck("name", name.toLowerCase());
    if (email) debouncedCheck("email", email.toLowerCase());
  }, [name, email]);
